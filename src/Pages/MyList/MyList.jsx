    ...
    const fetchTouristSpots = async () => {
      ...
      try {
        const response = await fetch(
          `https://tourism-management-server-gray.vercel.app/oneTopSpots?email=${user.email}`
        ); // hitting data on endpoint
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          fetch(
            `https://tourism-management-server-gray.vercel.app/delSpots/${id}`,
            {
              method: "DELETE",
            }
          )
